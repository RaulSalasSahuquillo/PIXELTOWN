...
from text_en import titulo, informaciontexto1, informaciontexto2
...
def escena_intro(pantalla, reloj):  # We need the clock to control the speed 
    try:
        ...
        print("Error: Video file 'intro.mp4' was not found in the visual folder. Skipping intro.")
    except Exception as e:
        print(f"An error occurred while playing the video: {e}. Skipping intro.")
    ...
    try:
        ...
        print(f"Could not load the music file: {e}")
...
def escena_menu(pantalla, fuente_titulo, fuente_boton, eventos):
    ...
    texto_surf = fuente_boton.render("PLAY", True, (0, 0, 0))
...
def escena_juego(pantalla, fuente_boton, eventos, fuente_titulo):
    ...
    for evento in eventos:
        ...
        if evento.type == pygame.MOUSEBUTTONUP:
            if boton_volver.collidepoint(evento.pos):
                ...
                print("Continuing to prompt scene...")
                return "preguntando"
    ...
    texto_surf_volver = fuente_boton.render("Back to Menu", True, (255, 255, 255))
    ...
    texto_surf_continuar = fuente_boton.render("Continue", True, (255, 255, 255))
...
def pregunta(pantalla, fuente_titulo, fuente_normal, eventos, caja_texto_estado, datos_jugador):
    ...
    for evento in eventos:
        ...
        if activo and evento.type == pygame.KEYDOWN:
            if evento.key == pygame.K_RETURN:
                print(f"Response submitted: {texto_usuario}")
    ...
    texto_pregunta_surf = fuente_titulo.render("What is your name?", True, (0, 0, 0))
    ...
    texto_surf_boton = fuente_normal.render("Back to Menu", True, (255, 255, 255))
...
def pregunta2(pantalla, fuente_titulo, fuente_normal, eventos, caja_texto_estado, datos_jugador):
    ...
    pantalla.fill((220, 220, 255))
    nombre_usuario = datos_jugador.get('nombre_usuario', 'You')
    texto_pregunta_surf = fuente_titulo.render(
        f"Welcome to PIXELTOWN, {nombre_usuario}! What is your city's name?", True, (0, 0, 0))
    texto_pregunta_rect = texto_pregunta_surf.get_rect(center=(pantalla.get_width() // 2, 200))
    pantalla.blit(texto_pregunta_surf, texto_pregunta_rect)
...
def cargamapa(pantalla, fuente_titulo, fuente_boton, eventos, fuente_normal):
    ...
    texto_titulo_surf = fuente_titulo.render("Loading Map...", True, (0, 0, 0))
...
def mapainicial(pantalla, fuente_titulo, fuente_boton, eventos, fuente_normal, datos_jugador):
    ...
    texto_surf_boton = fuente_normal.render("Actions", True, (255, 255, 255))
    ...
    mostrar_texto(pantalla, fuente_normal, f"Leader: {nombre_usuario}", 10, 10)
    mostrar_texto(pantalla, fuente_normal, f"City: {nombre_ciudad}", 10, 50)
    mostrar_texto(pantalla, fuente_normal, f"Money: {dinero}", 10, 90)
    mostrar_texto(pantalla, fuente_normal, f"Population: {poblacion}", 10, 130)
    mostrar_texto(pantalla, fuente_normal, f"Happiness: {felicidad}%", 10, 170)
    mostrar_texto(pantalla, fuente_normal, f"Buildings: {len(edificios)}", 10, 210)
    mostrar_texto(pantalla, fuente_normal, f"Experience: {experiencia}", 10, 250)
    ...
    try:
        ...
        print(f"Error loading building images: {e}")
    ...
    if felicidad < 10:
        print("Oh no! The citizens are not happy.")
        time.sleep(1)
        print("A coup d'état has started!")
        try:
            ...
            print("The city has been destroyed!\nYOU LOST")
            return "salir"
    ...
    for evento in eventos:
        ...
        if evento.type == pygame.MOUSEBUTTONUP:
            if boton_acciones.collidepoint(evento.pos):
                print("Changing to actions scene...")
                return "acciones"
...
def acciones(pantalla, fuente_titulo, fuente_boton, eventos, fuente_normal):
    pantalla.fill((255, 255, 255))  # White background (Maybe a bit too bright?)
    mostrar_texto(pantalla, fuente_titulo, "Actions", 10, 10)
    ...
    texto_surf_boton = fuente_normal.render("Buy", True, (255, 255, 255))
...
def adorno(pantalla, fuente_titulo, fuente_boton, eventos, fuente_normal):
    ...
    try:
        ...
        print(f"Could not load image 'AdornoMYTOWNMYRULES.png': {e}")
    ...
    texto_surf_boton = fuente_normal.render("Streetlight", True, (255, 255, 255))
    ...
    try:
        ...
        print(f"Could not load image 'farola.png': {e}")
    ...
    try:
        ...
        print(f"Could not load image 'arbusto.png': {e}")
    ...
    texto_surf_boton = fuente_normal.render("Bush", True, (255, 255, 255))
...
def productos(pantalla, fuente_titulo, fuente_boton, eventos, fuente_normal):
    ...
    color_boton = (200, 200, 100) if boton_comprar.collidepoint(pos_raton) else (200, 200, 50)
    pygame.draw.rect(pantalla, color_boton, boton_comprar)
    texto_surf_boton = fuente_normal.render("Lov'yc Hand Soap | 250", True, (255, 255, 255))
    texto_rect_boton = texto_surf_boton.get_rect(center=boton_comprar.center)
    pantalla.blit(texto_surf_boton, texto_rect_boton)
    for evento in eventos:
        if evento.type == pygame.MOUSEBUTTONUP:
    ...
    texto_surf_boton = fuente_normal.render("Lov'yc Gold Face Mask | 150", True, (255, 255, 255))
...
def productos2(pantalla, fuente_titulo, fuente_boton, eventos, fuente_normal):
    ...
    texto_surf_boton = fuente_normal.render("Lov'yc Shampoo | 200", True, (255, 255, 255))
    ...
    color_boton = (200, 200, 100) if boton_comprar.collidepoint(pos_raton) else (200, 200, 50)
    pygame.draw.rect(pantalla, color_boton, boton_comprar)
    texto_surf_boton = fuente_normal.render("Lov'yc Wipes | 100", True, (255, 255, 255))
    texto_rect_boton = texto_surf_boton.get_rect(center=boton_comprar.center)
    pantalla.blit(texto_surf_boton, texto_rect_boton)
    for evento in eventos:
        if evento.type == pygame.MOUSEBUTTONUP:
...
def construccion(pantalla, fuente_titulo, fuente_boton, eventos, fuente_normal):
    ...
    texto_surf_boton = fuente_normal.render("Simple House", True, (255, 255, 255))
    ...
    try:
        ...
        print(f"Could not load image 'supermercado.png': {e}")
    ...
    texto_surf_boton = fuente_normal.render("Supermarket", True, (255, 255, 255))
    ...
    try:
        ...
        print(f"Could not load image 'Casa.png': {e}")
    ...
    try:
        ...
        print(f"Could not load image 'tarraco.png': {e}")
...
def escena_colocacion(pantalla, eventos, fuente_normal, tipo_edificio):
    ...
    if tipo_edificio not in config_edificios:
        print(f"Error: Unknown building type '{tipo_edificio}'.")
        return "construccion"
    ...
    mostrar_texto(pantalla, fuente_normal, f"Money: {dinero}", 10, 10)
    mostrar_texto(pantalla, fuente_normal, f"Buildings: {len(edificios)}", 10, 40)
    mostrar_texto(pantalla, fuente_normal, f"Experience: {experiencia}", 10, 70)
    ...
    mostrar_texto(pantalla, fuente_normal,
                  f"Cost: {costo} | Left click: build | Right click: cancel",
        ...
        if evento.type == pygame.MOUSEBUTTONDOWN:
            if evento.button == 1:
                if dinero >= costo:
                    ...
                    print(f"{tipo_edificio.capitalize()} built at: {final_pos} | +{recompensa_exp} exp")
                    return "mapainicial"
                else:
...
def facturar(pantalla, fuente_titulo, fuente_boton, eventos, fuente_normal, datos_jugador):
    ...
    texto_surf_boton = fuente_normal.render("SELL BUILDING", True, (255, 255, 255))
    ...
    for evento in eventos:
        if evento.type == pygame.MOUSEBUTTONUP:
            if boton_vendeedificio.collidepoint(evento.pos):
                print("Changing to sell building scene...")
...
def impuestos(pantalla, fuente_titulo, fuente_normal, eventos, datos_jugador, caja_texto_estado, datos_impuestos):
    ...
    for evento in eventos:
        ...
        if activo and evento.type == pygame.KEYDOWN:
            if evento.key == pygame.K_RETURN:
                ...
                print(f"You collected {impuestoapagar} in taxes.")
    ...
    texto_pregunta_surf = fuente_titulo.render("What tax % do you want to charge citizens?", True, (0, 0, 0))
    texto_pregunta_rect = texto_pregunta_surf.get_rect(center=(pantalla.get_width() // 2, 200))
    pantalla.blit(texto_pregunta_surf, texto_pregunta_rect)
